...
export async function POST(request: Request) {
    try {
        ...
        if (user.resetTokenExpires) {
            const now = Date.now();
            const expiresTime = user.resetTokenExpires.getTime();

            // Hitung kapan email dikirim: Waktu expired dikurangi 15 menit masa aktif token
            const emailSentAt = expiresTime - (15 * 60 * 1000);
            // Batas waktu boleh resend adalah 1 menit setelah email terkirim
            const allowedTime = emailSentAt + (1 * 60 * 1000);

            if (now < allowedTime) {
                const diffMs = allowedTime - now;
                const diffSecs = Math.ceil(diffMs / 1000);

                return NextResponse.json({
                    error: `Harap Tunggu ${diffSecs} detik lagi sebelum meminta reset password kembali!`
                }, { status: 429 });
            }
        }
