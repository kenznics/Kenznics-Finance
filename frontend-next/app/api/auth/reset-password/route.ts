...
// Agar Vercel tidak meng-cache hasil validasi token GET
...
export async function POST(request: Request) {
    try {
        ...
        if (!user) {
            return NextResponse.json({ error: 'Token tidak valid atau sudah pernah digunakan!' }, { status: 400 });
        }
        ...
        return NextResponse.json({ message: 'Kata sandi berhasil diperbarui, silakan login kembali' });
    } catch (error) {
